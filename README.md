...
- Создание новой зоны и добавление в неё записей **с помощью Ansible**. Для создания зоны и добавления в неё записей, нужно добавить зону в файл /etc/named.conf на хостах ns01 [изменить master-named.conf](master-named2.conf) и ns02[изменить slave-named.conf ](slave-named2.conf), а также создать файл [named.newdns.lab](named.dns.lab), которые будут копироваться на сервер ns01 и ns02 через playbook.
...
>- После перезапуска файл /etc/resolv.conf перезаписывался
